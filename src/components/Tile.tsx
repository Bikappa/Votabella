...
export function Tile({
  ...
  return (
    <div
      ...
      tabIndex={-1}
    >
      {grade !== 10 && <IntervalIndicator progress={progress} />}
      <div
        className="relative flex h-full min-h-32 items-center justify-center rounded-xl p-6"
        style={{ background: bg }}
      >
        <button
          className={`absolute top-1 right-1 z-10 size-6 cursor-pointer opacity-70 hover:opacity-100 ${locked ? "" : controlClass}`}
          onClick={onLockToggle}
          title={locked ? "Unlock tile" : "Lock tile"}
        >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="flex flex-col w-full items-center justify-center gap-1">
            <input
              className="w-20 min-w-0 rounded bg-transparent px-1 text-sm text-gray-200 outline-none focus:bg-black/25 focus:outline focus:outline-1 focus:outline-white/60 text-center"
              value={label}
              onChange={(e) => onLabelChange(e.target.value)}
            />
            <span className="text-xl font-medium text-gray-300 text-shadow-xs">
              {title}
            </span>
          </div>
          <div className="flex gap-1 items-center">
            <div className={controlClass}>
              <DeltaButton
                disabled={grade <= minGrade}
                title="Decrease grade"
                onClick={() => onChange?.(grade - 0.25)}
              >
                <MinusIcon />
              </DeltaButton>
            </div>
